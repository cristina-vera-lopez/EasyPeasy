...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write('__________________________________________________________________________________________________________________________________')
			res.write('\n\n')
			...
			movies.sort(function (a, b) { //orden un array alfabeticamente por una propiedad del mismo
				if (a.title > b.title) {
					return 1;
				}
				if (a.title < b.title) {
				...
				return 0;
			}); //movies.sort((a, b) => (a.title > b.title)?1:(a.title < b.title)?-1:0); Otra forma de sort en arrow
			...
			res.write('\n')
			res.write(`
						                                                 >_ En cartelera          (/en-cartelera)
						                                                 >_ Más votadas           (/mas-votadas)
						                                                 >_ Sucursales            (/sucursales)
						                                                 >_ Contacto              (/contacto)
						                                                 >_ Preguntas precuentes  (/preguntas-frecuentes)
					  `)
			res.end();
			break;
		...
		case '/mas-votadas':
			...
			res.write('_______________________________________________________________________________________________________________________')
			res.write('\n\n')

			let masVotadas = movies.filter(function(elemento){
				return elemento.vote_average >= 7;
			});
			let rating = masVotadas.map((movies) => {
				return movies.vote_average;
			});
			let ratingTotal = rating.reduce(function(acum, rating) {
				return acum + rating;
			});
			let promedioRating = ratingTotal / masVotadas.length;
			movies.sort((a, b) => (a.title > b.title) ? 1 : (a.title < b.title) ? -1 : 0);
			...
			res.write('Rating promedio: ' + promedioRating.toFixed(2) + '\n \n');
			res.write('_______________________________________________________________________________________________________________________')
			res.write('\n\n')

		    movies.forEach(movies => {
			res.write('\n')
			res.write('>_TITULO: ' + movies.title + '\n');
			res.write('>_RATING: ' + movies.vote_average + '\n');
			res.write('>_RESEÑA: ' + movies.overview + '\n');
			})
			res.end();
			break;
		...
		case '/sucursales':
			...
			theaters.forEach(theaters => {
				res.write('>_ Nombre: ' + theaters.name);
				res.write('\n')
				res.write('>_ Ubicación: ' + theaters.address);
				res.write('\n')
				res.write('>_ Descripción: ' + theaters.description);
				res.write('\n\n')
			})
			res.end();
			break;
		...
		case '/preguntas-frecuentes':
			...
			faqs.forEach(function(faq) {
				res.write('>_ Pregunta: ' + faq.faq_title);
				res.write('\n')
				res.write('>_ Respuesta: ' + faq.faq_answer);
				res.write('\n\n')
			});
			res.end()
			break;
